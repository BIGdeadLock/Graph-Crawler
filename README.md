...
2) To submit several requests simultaneously, each crawler makes use of multithreading.
...
real or a spam email. In order to solve this problem I used TF-IDF to rank the email addresses. 
...
or a spam email address. If the email address is rare it is considered good, for example a professor's private email address.
...
# Testing - CI/CD
A testing framework was implemented. For future tests expanding, all future test need to implemented in the `testing` directory.
